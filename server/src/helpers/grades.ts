...
/**
 * Shifts a grade into a simplified difficulty number.
 *
 * @param {ClimbingGrade} grade Grade to convert.
 * @param {GradingSystem} system Specify the grading system.
 * @returns {number} Numerical value of grade.
 */
export const gradeToSimplifiedDifficultyIndex = (
  grade: ClimbingGrade,
  system = undefined as GradingSystem | undefined,
): number => {
  switch (system) {
    case GRADING_SYSTEMS.FRENCH:
      return FRENCH_ROUTE_SIMPLE_GRADES.indexOf(grade);
    case GRADING_SYSTEMS.YOSEMITE_DECIMAL_SYSTEM:
      return YOSEMITE_DECIMAL_SYSTEM_ROUTE_SIMPLE_GRADES.indexOf(grade);
    case GRADING_SYSTEMS.FONT:
      return FONT_BOULDER_SIMPLE_GRADES.indexOf(grade);
    default:
      return V_SCALE_SIMPLE_GRADES.indexOf(grade);
  }
};
...
/**
 * Retrieves simplified grade at an index.
 *
 * @param {number} index Grade to retrieve.
 * @param {GradingSystem} system Specify the grading system.
 * @returns {ClimbingGrade} Grade at that index.
 */
export const getGradeAtSimpleIndex = (
  index: number,
  system = undefined as GradingSystem | undefined,
): ClimbingGrade => {
  switch (system) {
    case GRADING_SYSTEMS.FRENCH:
      return FRENCH_ROUTE_SIMPLE_GRADES[index];
    case GRADING_SYSTEMS.YOSEMITE_DECIMAL_SYSTEM:
      return YOSEMITE_DECIMAL_SYSTEM_ROUTE_SIMPLE_GRADES[index];
    case GRADING_SYSTEMS.FONT:
      return FONT_BOULDER_SIMPLE_GRADES[index];
    default:
      return V_SCALE_SIMPLE_GRADES[index];
  }
};
