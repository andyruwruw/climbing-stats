...
export class Monitor {
  ...
  static log(
    ...
    if (Monitor._shouldLog(layer)) {
      console.log(
        `${Monitor.STD_OUT_MONITOR_LAYER_NAME_FORMATING[`${layer}`]}[${source.name}]:${Monitor.STD_OUT_ESCAPE_CODE_RESET}`,
        `${Monitor.STD_OUT_MONITOR_LAYER_MESSAGE_FORMATING[`${layer}`]}${text}${Monitor.STD_OUT_ESCAPE_CODE_RESET}`,
      );
    }
  }
